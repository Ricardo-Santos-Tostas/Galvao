...
set "DESTINO=%DESKTOP%\backup-banco"
if not exist "%DESTINO%" mkdir "%DESTINO%"

for /f %%i in ('powershell -NoProfile -Command "Get-Date -Format yyyy-MM-dd_HH-mm"') do set "STAMP=%%i"

set "ARQUIVO=%DESTINO%\backup_advocacia_%STAMP%.sql"
set "ULTIMO=%DESTINO%\backup_advocacia.sql"
...
powershell -NoProfile -Command ^
  "$dir = Join-Path ([Environment]::GetFolderPath('Desktop')) 'backup-banco';" ^
...
if "%SILENT%"=="0" (
    ...
    echo  Log: %LOG%
    echo  Mantem os ultimos 30 dias nesta pasta.
    echo ============================================
    pause
)
